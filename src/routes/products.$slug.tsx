...
export const Route = createFileRoute("/products/$slug")({
  ...
  head: ({ loaderData, params }) => ({
    meta: [
      { title: `${loaderData.product.name} | ${loaderData.category.name} | ASV Precision` },
      { name: "description", content: loaderData.product.description },
      { property: "og:title", content: `${loaderData.product.name} | ASV Precision` },
    ...
    scripts: [{
      ...
      children: JSON.stringify({
        ...
        brand: loaderData.product.brands.map((brand) => ({ "@type": "Brand", name: brand })),
    ...
    <>
        ...
        <div className="container-x grid lg:grid-cols-[1.1fr_0.9fr] gap-10 items-center">
          <div>
            <div className="text-xs text-white/60">
              <Link to="/" className="hover:text-white">Home</Link> / <Link to="/products" className="hover:text-white">Products</Link> / {product.name}
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {product.brands.map((brand) => (
                <span key={brand} className="glass rounded-full px-4 py-1.5 text-sm font-semibold text-white">{brand}</span>
              ))}
            </div>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/contact" className="inline-flex items-center gap-2 rounded-md bg-white px-7 py-3.5 font-semibold text-navy hover:bg-blue-50 transition shadow-xl">
                Request Quote <ArrowRight className="h-4 w-4" />
              </Link>
              <a href="mailto:sales.asvengg@gmail.com?subject=Catalogue%20Request%20-%20ASV%20Precision" className="inline-flex items-center gap-2 rounded-md border border-white/40 bg-white/5 px-7 py-3.5 font-semibold text-white hover:bg-white/15 transition">
      ...
      <section className="bg-white py-20">
        <div className="container-x grid lg:grid-cols-[1.15fr_0.85fr] gap-12">
          <div>
            <Reveal><h2 className="text-3xl font-extrabold text-navy">Product Gallery</h2></Reveal>
            ...
            <Reveal delay={0.08}><h2 className="mt-12 text-3xl font-extrabold text-navy">Applications</h2></Reveal>
            <Reveal delay={0.12}><p className="mt-4 text-mute leading-relaxed">{product.name} is suited for demanding machining environments where stability, repeatability and productivity matter. ASV Precision supports application matching based on component material, machine capability and production goals.</p></Reveal>

            <Reveal delay={0.16}><h2 className="mt-12 text-3xl font-extrabold text-navy">Technical Specifications</h2></Reveal>
            <div className="mt-6 grid gap-3">
              {product.features.map((feature) => (
                <div key={feature} className="rounded-xl border border-border p-4 text-sm text-ink flex items-center gap-3">
                  <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-brand-soft text-brand"><Check className="h-3.5 w-3.5" /></span>
                  {feature}
                </div>
              ))}
            </div>
              ...
              <div className="rounded-2xl border border-border bg-white p-7 shadow-sm">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {product.industries.map((industry) => (
                    <span key={industry} className="rounded-full bg-brand-soft px-3 py-1 text-xs font-semibold text-brand">{industry}</span>
                  ))}
                </div>
                ...
                <div className="mt-6 grid gap-3">
                  ...
                  <a href="https://wa.me/917338936926?text=Hi%2C%20I%27m%20interested%20in%20the%20product%20from%20ASV%20Precision." target="_blank" rel="noreferrer" className="inline-flex items-center justify-between rounded-xl border border-border p-4 font-semibold text-ink hover:bg-brand-soft transition">
      ...
      <section className="py-20" style={{ background: "var(--surface)" }}>
        <div className="container-x">
          <Reveal><h2 className="text-3xl font-extrabold text-navy">Related Products</h2></Reveal>
